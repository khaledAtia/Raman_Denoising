...
figure_name = 'validate_adenine_P2'
...
file_paths = [
    r'.\data\adenine\50_P.txt',
    r'.\data\adenine\40_P.txt',
    r'.\data\adenine\30_P.txt'
]
...
idx1=get_shift_indices(data1[:, 0],1600,1680)
idx2=get_shift_indices(data1[:, 0],1125,1150)
idx3=get_shift_indices(data1[:, 0],460,510)
alt_data1[idx1]=data3[idx1, 2]+data1[idx1, 2]
alt_data2[idx1]=data3[idx1, 2]+data2[idx1, 2]
alt_data2[idx2]=data1[idx2, 2]+data2[idx2, 2]
alt_data3[idx3]=data2[idx3, 2]
# Plots
ax_bl1.plot(data1[:, 0], data1[:, 1] - data1[:, 3], color='#9467bd', alpha=0.6, label='Raw spectrum-baseline(model)')
ax_bl1.plot(data1[:, 0], alt_data1, color='#8c564b', linestyle='-', label='Raman(model)')

ax_bl2.plot(data2[:, 0], data2[:, 1] - data2[:, 3], color='#9467bd', alpha=0.6)
ax_bl2.plot(data2[:, 0], alt_data2, color='#8c564b', linestyle='-')



#ax_bl2.plot(data2[:, 0], (data1[:, 2]/np.max(data1[:, 2]))* np.max( data2[:, 2]), color='#8c564b', linestyle='-')

ax_bl3.plot(data3[:, 0], data3[:, 1] - data3[:, 3], color='#9467bd', alpha=0.6)
ax_bl3.plot(data3[:, 0], alt_data3, color='#8c564b', linestyle='-')

# Vertical Shading for Target Raman Shifts

raman_shifts = [
    545, 633, 731, 988, 1075, 1135, 1205, 1257,
    1334, 1372, 1460, 1548
]
