...
def airPLS(x, lambda_=1e5, porder=2, itermax=40):
    '''
...
def airFit(x, lambda_=1e5, porder=2, itermax=40):
    m=x.shape[0]
    w=np.ones(m)
    for i in range(1,itermax+1):
        z=WhittakerSmooth(x,w,lambda_, porder)
...
def airPLSz(x, lambda_=1e5, porder=2, itermax=40):
    z=airPLS(x, lambda_, porder, itermax)
    return x-z
