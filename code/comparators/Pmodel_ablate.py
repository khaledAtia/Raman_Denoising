"""
GENERAL ABLATION MODEL -- a superset of Pmodel.py.

Every constructor default reproduces Pmodel.py exactly (verified: identical parameter
count, identical weights under a common seed, identical forward outputs). Each flag
switches off or reparameterises one component so that the module ablations of the
Supporting Information can be produced from a single code path.

    hint_mode        'subtract'      hint = E_l - B_l                (default, as reported)
                     'concat'        hint = [E_l || B_l]
                     'encoder-only'  hint = E_l  (baseline branch does not gate)

    gate_sigmoid     True            evidence is multiplied by a sigmoid gate (default)
                     False           evidence passes ungated; the gate-projection
                                     convolution is not constructed at all, so no dead
                                     parameters are counted

    squelch          True            terminal SpatialSquelchGate applied (default)
                     False           the raw signal projection is returned directly

    base_latent_dim  16              channels routed to the baseline branch at the
                                     bottleneck; the remainder go to the Raman branch

    widths           (64,128,256,512) encoder channel schedule. Successive entries must
                                     double, because each decoder Up block concatenates a
                                     skip of half the incoming width.

NOTE ON SCOPE
    This file supersedes Pmodel_gate.py, which implemented hint_mode alone and was used
    for the two completed gate runs. Pmodel_gate.py is retained unchanged so those runs
    remain exactly reproducible; new runs should use this file. Constructed with
    hint_mode='concat' this model is architecturally identical to Pmodel_gate.py with the
    same setting.

    The loss-side ablations -- removing the orthogonality penalty or the auxiliary deep
    supervision -- require no model change and are applied in train_ablation.py by setting
...
class HighResPhysicsGate(nn.Module):
    ...
    """
    def __init__(self, current_channels, deep_channels, hint_channels, fusion_mode='concat', kernel_size=3, temperature=1.0, bias_value=None, use_sigmoid=True):
        super(HighResPhysicsGate, self).__init__()

        # ABLATION: when use_sigmoid is False the evidence is passed ungated and the
        # gate-projection convolution is not constructed, so it contributes no parameters.
        ...
        # ==========================================
        if not use_sigmoid:
            pass  # ABLATION: no gating branch is built at all
    ...
    def forward(self, current_signal, deep_signal, current_hint):
        ...
        if not self.use_sigmoid:
            gate = None                      # ABLATION: evidence passes ungated
        elif self.fusion_mode == 'concat':
        ...
        # --- C. CROSS-GATING ---
        confirmed_evidence = evidence * gate if self.use_sigmoid else evidence
...
class AUSequentialUNet(nn.Module):
    ...
    def __init__(self, n_channels=1, bilinear=False, gamma=1, kernels=[1, 3, 5],base_latent_dim=32, use_derivatives=True,
                 hint_mode='subtract', gate_sigmoid=True, squelch=True,
                 widths=(64, 128, 256, 512)):
        super(AUSequentialUNet, self).__init__()

        # ---- ABLATION FLAGS (defaults reproduce Pmodel.py exactly) ----
        ...
        self.use_squelch = squelch
        w0, w1, w2, w3 = widths
        if (w1, w2, w3) != (2 * w0, 4 * w0, 8 * w0):
            raise ValueError("widths must double at each stage; each Up block "
                             "concatenates a skip of half the incoming width")
        if base_latent_dim >= w3:
            raise ValueError("base_latent_dim must be smaller than the bottleneck width")
        ...
        self.inc = RK4SmoothedBlock(augmented_channels, w0, smooth_skip=False, pool_kernel=3)
        self.down1 = Down(w0, w1, smooth_skip=False, pool_kernel=3)
        self.down2 = Down(w1, w2, smooth_skip=False, pool_kernel=3)
        self.down3 = Down(w2, w3, smooth_skip=False, pool_kernel=3)
        
        # ==========================================
        # LATENT SPACE DISENTANGLEMENT (The Split)
        # ==========================================
        self.base_latent_dim = base_latent_dim
        self.sig_latent_dim = w3 - self.base_latent_dim
        ...
        self.up1_base_conv = SmoothedResBlock(self.base_latent_dim + w2, w2, smooth_skip=False)
        ...
        self.up1_sig_conv = SmoothedResBlock(self.sig_latent_dim + w2, w2, smooth_skip=False)

        # --- BASELINE HEAD (Stages 2 & 3 remain symmetric) ---
        self.up2_base = Up(w2, w1, bilinear, smooth_skip=False)
        self.up3_base = Up(w1, w0, bilinear, smooth_skip=False,pool_kernel=3)
        self.out_base = nn.Conv1d(w0, n_channels, kernel_size=1)

        # --- BASELINE AUXILIARY HEADS (Deep Supervision) ---
        self.out_base_deep = nn.Conv1d(w2, n_channels, kernel_size=1)
        self.out_base_mid = nn.Conv1d(w1, n_channels, kernel_size=1)
        ...
        # Stage 2: Mid (256 -> 128)
        self.up2_sig = Up(w2, w1, bilinear)
        ...
        self.up3_sig = Up(w1, w0, bilinear,smooth_skip=False,pool_kernel=5)
        ...
        self.out_sig = nn.Conv1d(w0, n_channels, kernel_size=1)

        # --- SIGNAL AUXILIARY HEADS (Deep Supervision) ---
        self.out_sig_deep = nn.Conv1d(w2, n_channels, kernel_size=1)
        self.out_sig_mid = nn.Conv1d(w1, n_channels, kernel_size=1)

        # Squelch Gate
        #self.gate = SquelchGate(in_channels=64, init_val=0.0)
        self.gate = SpatialSquelchGate(in_channels=w0, kernel_size=3,
                                       temperature=1.0,
                                       bias_value=None) if squelch else None
    ...
    def forward(self, x):
        ...
        # --- STAGE 1 GATING ---
        hint_deep = self._hint(x3, b_deep)
        ...
        hint_final = self._hint(x1, b_final)
        ...
        # Squelch Gate 
        final_signal = self.gate(s_final, raw_signal) if self.use_squelch else raw_signal
