"""
ABLATION VARIANT of Pmodel.py  --  "plain residual encoder".

Identical to Pmodel.py in every respect EXCEPT the encoder residual block:

    Pmodel.py            encoder uses RK4SmoothedBlock  (4th-order Runge-Kutta update:
                                                         z + h/6*(k1 + 2k2 + 2k3 + k4))
    Pmodel_plainres.py   encoder uses SmoothedResBlock  (plain pre-activation ResNet-V2:
                                                         shortcut(x) + F(x))

Two call sites differ, both marked with "# ABLATION:":
    - Down.__init__            (line ~319)  -> used by down1/down2/down3
    - AUSequentialUNet.inc     (line ~403)

Everything else -- channel widths (64/128/256/512), the 16/496 bottleneck split, the
HighResPhysicsGate hint gating, SpatialSquelchGate, deep supervision heads, the decoder
(which already used SmoothedResBlock in both versions), ContrastiveLoss and EarlyStopping
-- is byte-identical to Pmodel.py.

RK4SmoothedBlock is left defined but unused, so this file stays a faithful diff of the
original and can be swapped back by reverting the two marked lines.

Note: the RK4 block evaluates conv_block 4x per forward pass, so this variant is roughly
4x cheaper per encoder block. Report parameter count AND wall-clock time alongside the
accuracy numbers -- a plain-residual model that matches RK4 accuracy at lower cost is a
result, not a failure.

Train with:  train_ablation.py --arch plain
Answers reviewer comments R1.1 / R2.2 / R3.7 (module ablation).
"""
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, smooth_skip=False, pool_kernel=3):
        ...
        self.down = nn.Sequential(
            ...
            # ABLATION: RK4SmoothedBlock -> SmoothedResBlock (plain pre-activation residual)
            SmoothedResBlock(in_channels, out_channels, smooth_skip=smooth_skip, pool_kernel=pool_kernel)
        )
...
class AUSequentialUNet(nn.Module):
    def __init__(self, n_channels=1, bilinear=False, gamma=1, kernels=[1, 3, 5],base_latent_dim=32, use_derivatives=True):
        ...
        # ABLATION: RK4SmoothedBlock -> SmoothedResBlock (plain pre-activation residual)
        self.inc = SmoothedResBlock(augmented_channels, 64, smooth_skip=False, pool_kernel=3)
        ...
        self.attn1 = HighResPhysicsGate(
                    ...
                    hint_channels=256,  # Note: The hint is 128 here, the math handles it safely!
        ...
        self.attn2 = HighResPhysicsGate(
                    ...
                    deep_channels=256, 
                    hint_channels=128,  # Note: The hint is 128 here, the math handles it safely!
                    fusion_mode='concat',  # <--- Toggle between 'concat' and 'sum'
                    temperature=temp,
                    bias_value=gate_bias,
                    kernel_size=kernels[1]
        ...
        self.attn3 = HighResPhysicsGate(
                    current_channels=64, 
                    deep_channels=128, 
                    hint_channels=64,  # Note: The hint is 128 here, the math handles it safely!
                    fusion_mode='concat',  # <--- Toggle between 'concat' and 'sum'
                    temperature=temp,
                    bias_value=gate_bias,
                    kernel_size=kernels[1]
